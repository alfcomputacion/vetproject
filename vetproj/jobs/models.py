...
class Aplicant(models.Model):
    ...
    website = models.URLField(blank=True, validators=[
                              URLValidator(schemes=['http', 'https'])])
    employment_type = models.CharField(choices=EMPLOYMENT_TYPES, max_length=10)
    start_date = models.DateField(help_text='The earliest you can start working',
                                  validators=[validate_future_date])
    available_days = models.CharField(max_length=20)
    desired_hourly_wage = models.DecimalField(max_length=50, decimal_places=2)
